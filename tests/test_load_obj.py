...
class TestCore(unittest.TestCase):
    def test_tetrahedron(self):
        ...
        assert (np.allclose(jt.array(vertices_ref).numpy(), vertices.numpy()))
        assert (np.allclose(jt.array(faces_ref).numpy(), faces.numpy()))
        vertices, faces = nr.load_obj(obj_file, True)
        assert (np.allclose(jt.array(vertices_ref).numpy() * 2 - 1.0, vertices.numpy()))
        assert (np.allclose(jt.array(faces_ref).numpy(), faces.numpy()))
    ...
    def test_texture(self):
        ...
        images, _, _ = renderer.render(vertices.unsqueeze(0), faces.unsqueeze(0), textures.unsqueeze(0))
